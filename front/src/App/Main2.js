        ...
        axios({
            ...
            if(res.data){
                res.data.result.map((e,i)=>{
                    // console.log(e);
                    content_arr[i] = {roomName : e.roomName, existence : e.existence, title : e.title, src : 'data:image/jpeg;base64,'+res.data.base64_img_arr[i], uuid : e.uuid}
                })
            ...
            <div className="queze_list">
                {
                    main_content_state.map((e,i)=>{
                        return(
                            <Main2_content key={i} title={e.title} roomName={e.roomName} existence={e.existence} src={e.src} uuid={e.uuid}></Main2_content>
                        )
                    })
                }
            </div>
