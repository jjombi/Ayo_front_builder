...
const Main2_content = ({roomName, src, title, existence, uuid, password, explain_text}) => {
    ...
    const A_queze_click = (roomName) => {
        ...
        if(password === ''){
            navigate(`/choosequezetype?roomName=${roomName}&title=${title}&explain_text=${explain_text}`);
        }
        else{
            setPopup_state(true);
            setType('queze');
    ...
    const result_click = (roomName) => {
        ...
        if(password === ''){
            navigate(`/result?roomName=${roomName}&title=${title}&explain_text=${explain_text}`);
        }
        else{
            setPopup_state(true);
            setType('result');
