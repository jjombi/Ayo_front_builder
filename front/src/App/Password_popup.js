...
const Password_popup = ({setPopup_state , uuid, roomName, title, publicAccess, type, typeWhere}) => {
    ...
    const password_checker = () => {
        if(typeWhere === 'modify_password'){
            axios({
                ...
                console.log(res);
                if(res.data === 'success'){
                    navigate(`/makeaquezemodify?roomName=${roomName}&title=${title}`,{state:{tinyint : true}});
                }else{
                    alert('비번 불일치');
                }
            })
